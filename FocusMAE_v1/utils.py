...
def multiple_samples_mask_collate(batch, fold=False):
    """
    Collate function for repeated augmentation. Each instance in the batch has
    more than one sample.
    Args:
    ...
    Returns:
        (tuple): collated data batch.
    """
    inputs, labels, video_idx, mask,extra_data = zip(*batch)
    inputs = [item for sublist in inputs for item in sublist]
    labels = [item for sublist in labels for item in sublist]
    video_idx = [item for sublist in video_idx for item in sublist]
    if len(mask):
    ...
    inputs, labels, video_idx,mask,extra_data = (
        default_collate(inputs),
        default_collate(labels),
        default_collate(video_idx),
        default_collate(mask),
    ...
    if fold:
        return [inputs], labels, video_idx, mask,extra_data
    else:
        return inputs, labels, video_idx, mask,extra_data
...
def multiple_pretrain_samples_collate(batch, fold=False):
    ...
    Args:
        batch (tuple or list): data batch to collate.
    Returns:
        (tuple): collated data batch.ssss
    """
    # print(batch)
    process_data, encoder_mask, decoder_mask , vid, fname= zip(*batch)
    # print(len(process_data), process_data[0])
    ...
    vid = [sublist for sublist in vid]
    fname = [item for sublist in fname for item in sublist]
    process_data, encoder_mask, decoder_mask, vid, fname  = (
        default_collate(process_data),
        default_collate(encoder_mask),
        default_collate(decoder_mask),
        default_collate(vid),
        default_collate(fname),
    )
    if fold:
        return [process_data], encoder_mask, decoder_mask, vid, fname
    else:
        return process_data, encoder_mask, decoder_mask , vid, fname


def multiple_pretrain_samples_train_collate(batch, fold=False):
    """
    Collate function for repeated augmentation. Each instance in the batch has
    more than one sample.
    Args:
