...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def get_mask(self, x, priors, prior_mask,delta):
        ...
        # p_x = self.softmax(logits, dim=-1)
        # p_x = torch.nn.functional.softmax(logits, dim=-1)
        p_x = self.softmax(logits)
        # print('p_x: ',p_x)
        # print('p_x.shape: ',p_x.shape)
        # print('torch.sum(p_x): ',torch.sum(p_x))
        
        # print("Sum of p_x along dim=-1: ", torch.sum(p_x, dim=-1))  # Should be ~1 for each row       
        # print('p_x.device: ',p_x.device)
        # print("Min value in p_x:", torch.min(p_x))
        # print("Type of p_x:", type(p_x))
        # print("Is p_x a meta tensor?", p_x.is_meta)
        k=p_x.is_meta
...
def pretrain_adamae_base_patch16_224(pretrained=False, mask_ratio=0.9, **kwargs):
    model = PretrainVisionTransformer(
        img_size=224,
        patch_size=16, 
        encoder_embed_dim=768, # Changed from 384 to 768
        encoder_depth=12, 
        encoder_num_heads=12,
        encoder_num_classes=0,
        decoder_num_classes=1536,
