...
def get_args():
    ...
    parser.add_argument(
    '--test_randomization', action='store_true', default=True)
...
def extract_encoder_weights(pretrained_dict, model_dict):
    ...
    for key in model_dict.keys():
        pretrained_key = f"_orig_mod.encoder.{key}"  # Construct the corresponding key
...
def main(args, ds_init):
    ...
    # print("Before Model embed_dim:", model.embed_dim)
    # # Manually change embedding dimension
    # model.patch_embed.proj.out_channels = 768  # Change projection layer
    # model.embed_dim = 768  # Update embedding dimension
    # model.head.in_features = 768  # Update classifier head input
    # print("After Model embed_dim:", model.embed_dim)  # Should be 768
    ...
    if args.finetune:
        ...
        #         checkpoint_model = new_state_dict[model_key]
        #         # checkpoint_model = {f"encoder.{k}": v for k, v in checkpoint_model.items()}
        #         print("Load state_dict by model_key = %s" % model_key)
        #         break
        # print('###checkpoint_model: ',checkpoint_model) 
        ...
        if checkpoint_model is not None:
            ...
            checkpoint_model_keys = set(checkpoint_model.keys())
            same_keys = model_keys.intersection(checkpoint_model_keys)
            different_model_keys = model_keys - checkpoint_model_keys
            different_checkpoint_keys = checkpoint_model_keys - model_keys

            # Print results
            print("\nKeys present in both model and checkpoint_model:")
            print(", ".join(same_keys))

            print("\nKeys only in model but not in checkpoint_model:")
            print(", ".join(different_model_keys))

            print("\nKeys only in checkpoint_model but not in model:")
            print(", ".join(different_checkpoint_keys))
        else:
            print("No matching model_key found in the checkpoint.")
        ...
        # utils.load_state_dict(model, checkpoint_model, strict=False)
    ...
    if args.enable_deepspeed:
        ...
        print("THIS IS THE ELSE BLOCK, ", args.distributed)
        if args.distributed:
            print("THIS IS THE ELSE BLOCK inside IF, ", args.distributed)
    ...
    if args.eval:
        ...
        if global_rank == 0:
            ...
            final_top1, final_top2 = merge(args.output_dir, num_tasks)
            print(
                f"Accuracy of the network on the {len(dataset_test)} test videos: Top-1: {final_top1:.2f}%, Top-5: {final_top2:.2f}%"
            )
            log_stats = {'Final top-1': final_top1, 'Final Top-2': final_top2}
